...
def load_finetuned_pipeline():
    ...
    global TRANSFORM_META, INPUT_TRANSFORM, OUTPUT_TRANSFORM, ZERO_FLOOR, MAX_OUTPUT
    ...
    MAX_OUTPUT       = TRANSFORM_META["max_output"]
...
def predict_quantiles_for_origins(pipe, df_full, id_col, cpu_col,
    ...
    for batch_start in range(0, len(contexts_to_predict), batch_size):
        ...
        # SIDECAR: inverse transform back to raw scale + domain-bound clipping.
        # For no-log1p adapters, OUTPUT_TRANSFORM == "none" + MAX_OUTPUT == None → identity.
        # For log1p adapters with max_output set, expm1 outputs are capped at max_output.
        q_stack = apply_output_inverse(q_stack, OUTPUT_TRANSFORM, zero_floor=ZERO_FLOOR, max_output=MAX_OUTPUT)
