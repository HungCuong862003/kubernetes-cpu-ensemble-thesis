# %% [markdown]
# # supplementary_metrics.py — Seasonal naive, MASE, Krogh-Vedelsby, MDA.
#
# Supplementary evaluation metrics beyond the main R²/MAE analysis.
#
# Parts:
#   1. MASE denominators (per-container scaling factors from training set)
#   2. Seasonal naive baseline (288-step period = 1 day at 5-min intervals)
#   3. Krogh-Vedelsby BVC decomposition (bias-variance-covariance in residual space)
#   4. MDA — Mean Directional Accuracy (filtered for zero-change samples)
#
# Inputs:
#     train.parquet, test.parquet                (ALI_TRAIN, ALI_TEST)
#     per_container/<hz>/*.npy                   (PER_CONTAINER)
#     pred_*.npy from results/alibaba/<hz>/      (ALI_RESULTS)
#
# Outputs:
#     supplementary tables printed to console + checkpoint JSONs
#
